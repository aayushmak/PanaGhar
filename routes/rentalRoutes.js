...
// ✅ Payments Page - shows only rented/overdue books
router.get("/payments", async (req, res) => {
  try {
    ...
    res.render("payment", { books, user: req.session.user });
  } catch (err) {
    console.error("Error loading payment page:", err);
    res.status(500).send("Server error loading payment data");
  }
});
...
router.get("/esewa/success", async (req, res) => {
  try {
    ...
    if (!bookId) {
      console.error("❌ No bookId received in success URL");
    ...
    if (req.session.user) {
      book.rentedBy = req.session.user._id; // ✅ assign current user
    }

    await book.save();

    console.log(`✅ Payment successful & book updated: ${bookId}`);
